...
| **Дизеринг** | Флойд–Стейнберг, Аткинсон, синий шум, Байер 4×4, с необязательным подчёркиванием краёв |
...
| **PNG** | рисуется по точкам, поэтому в него не попадают ни отступы шрифта, ни полые кружки |
...
### Картинка на выходе рисуется, а не набирается
...
На экране с этим ничего не сделать — страница показывает то, что даёт шрифт.
Но выгружаемое изображение наше от начала до конца, поэтому PNG рисуется по
точкам: две поперёк ячейки и четыре вдоль, каждая в середине своей четверти, —
так что расстояние внутри ячейки и расстояние между ячейками это одно и то же
число. Ячейка сохраняет пропорции, под которые арт и раскладывался, поэтому
картинка имеет обещанную экраном форму, но без просветов, с которыми экран
...
отрисуется он тем моноширинным шрифтом, какой окажется у зрителя. Об этом там
сказано полностью.
