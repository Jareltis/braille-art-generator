...
| **Дизеринг** | переменные коэффициенты (обычные и со сдвигом порога), Флойд–Стейнберг, Аткинсон, синий шум, Байер 4×4, с необязательным подчёркиванием краёв |
...
| **Символы** | точки Брайля или сплошные блоки Unicode 16 — там, где у читающего есть шрифт |
...
| **Цвет** | одна краска на ячейку или две — чернила и фон — везде, где есть что красить |
...
Растр, из которого всё это считается, не фиксированного размера, и об этом
стоит сказать. Раньше он был ограничен двумя миллионами пикселей, а ровный
счёт пикселей значит разное на разных сетках: шестнадцать отсчётов на точку у
арта в сто столбцов и 1.6 у арта в четыреста — это чуть больше, чем просто
прочитать пиксель, и никакой статистики, ради которой весь этот раздел и
написан. Поэтому предел следует за сеткой: **не меньше четырёх отсчётов на
точку**, по два в каждую сторону, но не больше, чем есть в исходнике, и не
больше двадцати мегабайт растра.

Замер на фотографии 4912×7360 при сетке 400×400, против эталона, снятого так
подробно, как картинка позволяет: 1.56 отсчёта на точку давали 0.9041, четыре
дают 0.9221. Дальше берут больше, чем отдают: 8.3 отсчёта — 0.9140, полные
шестнадцать — 0.9211 на менее крайней сетке, ценой шестикратного времени и
шестидесяти мегабайт.

Пол срабатывает только выше полумиллиона точек — это сетка крупнее примерно
250×250 ячеек; при обычных пропорциях четыреста столбцов дают от 130 до 260
строк, и ничего не меняется. Там, где срабатывает, он стоит денег: один пересчёт
того самого арта 400×400 идёт 569 мс вместо 243, а подбор вариантов, который
кодирует и оценивает десяток рецептов на том же растре, — 30 секунд вместо 13.5.
Это цена того, что плитка показывает ровно тот арт, который получится: искать на
картинке грубее, чем та, для которой ищем, нельзя.
...
сама.
**Переменные коэффициенты со сдвигом порога** — продолжение Островоухова, сделанное
Чжоу и Фаном: та же по форме таблица, пересчитанная, плюс вторая, которая говорит,
насколько сильно на каждом уровне можно двигать порог. Одни переменные коэффициенты
оставляют в полутонах регулярный узор, а зависящий от уровня случайный сдвиг его
ломает — и делает это безопасно: диффузия ошибки всё равно меряет ошибку от истинного
значения, поэтому окрестность возвращает тон. По замеру на шести картинках при сорока
и шестидесяти столбцах способ набирает больше обычных коэффициентов в девяти случаях
из двенадцати (до +0.011) и меньше в трёх (до −0.014). Все проигрыши — на одном и том
же рисунке, где сдвиг порога сыплет шум в плоский цвет; поэтому способ предлагается,
но умолчанием не становится: по здешнему правилу для этого нужно выигрывать везде.
Сдвиг засеян, так что дважды подряд картинка выходит одна и та же.
...
Основную часть этой цены раньше платили там, где подчёркивать было нечего:
высокочастотная часть отвечает на крапины ровно так же, как на край, и на полной
силе порогом гуляли по всему склону холма впустую. Теперь она масштабируется тем,
сколько структуры на месте на самом деле, — тем же градиентом после лёгкого
размытия, на который опирается смешивание деталей, и это дёшево: плоскость уже
сведена к одному значению на точку. На ступеньке, утопленной в крапинах той же
силы, наклон в крапинах падает с 5.5 до 0.1, а на ступеньке остаётся 22.0; по
шести картинкам при сорока и шестидесяти столбцах вариант с гейтом набирает не
меньше прежнего в десяти случаях из двенадцати. Гейт вдвое ослабляет наклон и на
самом краю, поэтому сила несёт измеренный множитель два — чтобы верх ползунка
остался там же, где был.
...
Точка либо поднята, либо нет. У арта один бит там, где у фотографии восемь, и
картинка, живущая в нижней части своего диапазона, тратит этот бит впустую:
тёмный лес приезжает пустым кадром с единственным светлым пятном.

Общая на весь кадр кривая, которую предлагают остальные ползунки, этого не
чинит, и это измерено, а не предположено. Против глобального поднятия, подобранного
до той же средней яркости, по числу ячеек, не несущих никаких сведений — все точки
погашены или все подняты, и то и другое говорит одно и то же ничто, — глобальное
на двух картинках из четырёх сделало **хуже**, обменяв плоское чёрное на плоское
белое:

| картинка | без правки | глобально, та же яркость | тени и света |
|---|---|---|---|
| тёмный лес | 56.2% | 48.0% | **41.7%** |
| жёсткая графика | 19.5% | 25.9% | **17.1%** |
| тёмный композит | 45.8% | 45.5% | **42.8%** |
| пейзаж | 24.6% | 27.8% | **15.3%** |

Поэтому два ползунка спрашивают у каждого пикселя другое: не насколько он яркий,
а насколько он яркий рядом с тем, что его окружает. Коэффициент, собранный из
окрестности, умножает **линейный свет** всех трёх каналов одинаково, поэтому
поднятая тень сохраняет свой цвет, а не уходит в серое, и спадает он как куб
расстояния от своего конца шкалы — по замеру против квадрата, четвёртой и шестой
степеней куб лучший в среднем и худший ни в одном случае, и он самый мягкий из
тех, что не пускают ползунок теней в освещённое небо.

Окрестность считается по картинке в восемь раз меньше и размывается там же:
маске достаточно быть гладкой, а широкое размытие двухмегапиксельного растра —
самая дорогая вещь в этом проекте. Весь проход стоит 15–36 мс. Читается маска
между своими отсчётами, а не блоками: ступенька в коэффициенте видна
прямоугольником. Радиус задан долей кадра, а не числом пикселей, поэтому одна и
та же настройка значит одно и то же при любом размере растра.
...
### Та же ячейка, записанная блоками

Брайль выбран потому, что он есть у всех и потому, что его ячейка — ровно две
точки поперёк на четыре вдоль. В 2024 году Unicode 16 добавил **октанты**: ту же
ячейку 2×4, нарисованную сплошными блоками, вообще без просветов. Та же сетка,
тот же кодировщик — и в терминале со шрифтом это попросту лучшая картинка.

Арт при этом остаётся брайлевым. Блоки — способ его записать: восьмибитный узор
ячейки переводится в символ, а всё остальное в приложении продолжает работать с
тем же брайлем. Соответствие — само юникодовское: 230 символов U+1CD00…U+1CDE5
по возрастанию узора, минус двадцать шесть узоров, которые уже покрыты старыми
блоками (полный блок, половины, четверти). У всех 256 узоров есть символ, и ни
один не повторяется — это проверяется тестом.

Загвоздка — шрифт, и она настоящая: по замеру на Windows 11 в 2026 году ни
собственный стек шрифтов приложения, ни просто `monospace` не рисуют ни одного
октанта, тогда как брайль и четверти есть везде. Поэтому страница проверяет,
прежде чем предлагать: рисует символ и тот единственный код, для которого глифа
нет нигде, и сравнивает пиксели. Где шрифта нет — пункт выключен и сказано
почему. Правка точек на время блоков уходит: она считает точки в брайлевой
ячейке.

Что стоит знать при вставке: блок вне основной плоскости считается за два знака
там, где лимит считает по UTF-16, — счётчик считает так же.
...
SVG теперь тоже рисует точки — до определённого предела. Нарисованный — картинка
лучше: та же ровная решётка, шрифта в цепочке нет. Стоит это около 27 байт на
точку. По замеру на пейзаже, где поднята примерно треть точек: сорок столбцов —
118 КБ, шестьдесят — 259, сто двадцать — мегабайт, четыреста — одиннадцать
мегабайт, то есть файл, который никто не откроет. Поэтому граница — сорок тысяч
поднятых точек. Ниже неё SVG состоит из фигур и выглядит везде одинаково; выше —
арт пишется текстом: несколько килобайт, которые остаются выделяемыми и правимыми
ценой того, что отрисуются шрифтом зрителя, вместе со всеми просветами. Строка
состояния говорит, что именно сохранилось.
...
Всё, что можно сделать с готовым артом, раньше жило в колонке настроек — одним
плоским рядом из тринадцати кнопок между «измерить свой чат» и контурными
ползунками. Плохо в этом было двоякое. Кнопки стояли далеко от того, над чем
работают: на телефоне 390×844 **«Копировать»** оказывалась примерно на 1700-м
пикселе, двумя экранами ниже арта. И все они выглядели одинаково важными,
поэтому ряд приходилось прочитывать с начала каждый раз.

Теперь они под артом, тремя группами в том порядке, в каком к ним тянутся: что
сделать с ним (копировать, картинкой, поделиться, ссылка), что сделать над ним
(подобрать, пересчитать, назад, вперёд, править точки) и что сохранить.
Сохранения — не кнопки, а фишки: файл это меньший поступок, чем копирование. На
том же телефоне **«Копировать»** теперь около 690-го пикселя — на первом экране,
и арт над ней.

Строка состояния переехала вместе с ними. Раньше она отвечала из левого верхнего
угла, пока кнопка была внизу средней колонки; теперь она в том же ряду, там, где
и так смотрит глаз. В шапке осталось то, что нужно до появления арта: режим и
язык. **Расположение** ушло в группу «Отображение» — его выставляют один раз, а
на телефоне оно и вовсе ничего не делает.

### Светлая и тёмная

Страница была тёмной, что бы ни говорила система, — плохой ответ и тому, у кого
система говорит обратное, и тем глазам, которым светлое по тёмному читать тяжело.
Теперь она следует `prefers-color-scheme`, а в поле **«Тема»** обе можно выбрать
явно.

Палитра и так была набором переменных, поэтому светлая тема — это те же
переменные, прочитанные наоборот: бумага вместо чернил. Акцент взят темнее —
зелёный, который несёт белый текст, не несёт чёрный по белому. Оба набора
выписаны дважды, а не переключаются классом: класс ставится скриптом, а скрипт
приходит поздно, и страница успела бы показать себя тёмной на кадр.

Арт следует за этим сам. Точки рисуются цветом `--ink`, а экспорты читают те же
свойства со страницы, — значит светлая страница означает тёмные точки: и в
панели, и в PNG, и в том, что копируется картинкой. Трём фиксированным тёмным
заливкам пришлось сперва стать переменной: колодец, в котором лежит превью, был
третью чёрного, а это на белой странице серая плита.

### На любом экране

Замерено на двенадцати размерах, а не предположено: сложенный телефон 320×568,
маленький 360×640, обычные 390×844 и 430×932, телефон боком 844×390, планшеты в
обеих ориентациях, ноутбук, 1920×1080, ультраширокий 2560×1440, низкое окно
1440×620 и высокое узкое 480×1200.

Что проверяется на каждом: не появляется ли горизонтальная прокрутка (нигде),
держит ли десктоп всё без прокрутки вообще (да, начиная с 1024×768) и попадает ли
на первый экран то, ради чего всё затевалось, — «Копировать». Вот это не
выполнялось ровно в двух местах, и оба низкие: на сложенном телефоне кнопка
оказывалась на 65 пикселей ниже сгиба, на телефоне боком — на 81.

Палец — не курсор, а кнопки были сделаны под курсор: самая маленькая 36×30, а
флажок 16×16 — против 44, которые просит Apple, и 48 у Material. Там, где
указатель грубый (это и есть верный вопрос: сенсорный ноутбук бывает шириной
1440, а узкое окно на десктопе никто пальцем не трогает), у контролов пол в 44,
у второстепенных фишек 40, у флажков 24.

Само правило — медиазапрос, которого этот браузер не выполняет: эмуляция
маленького экрана не делает страницу сенсорной. Поэтому набор тестов достаёт
правила из запроса, накладывает их по-настоящему и меряет — так проверяется, что
они *делают*, а не что они написаны. Удалить их — и самая маленькая кнопка падает
с 44 до 29; это проверено удалением.

Это стоит высоты, а высоты низким экранам и так не хватало: на 44 кнопка снова
ушла за сгиб на телефоне 360×640. Поэтому правила для низких экранов начинаются
не с 620 пикселей высоты, а с 700, и ниже 430 есть третья ступень, которая
отдаёт ещё. Все двенадцать размеров держат кнопку на первом экране и с пальцем на
стекле.

Размеры шрифта заданы в `rem`, а не в пикселях, — поэтому читателю, который
попросил браузер о более крупном тексте, крупный текст и достаётся. Пиксели эту
настройку игнорируют вовсе (масштаб страницы — другое дело, он работал всегда).
Чего при этом не должно случиться — так это развала страницы, поэтому высоту
отдаёт арт: на 150% он ужимается с 371 до 268 пикселей и кнопка остаётся на
экране, на 200% — с 371 до 320, и она всё ещё там. На десктопе не отдаётся ничего:
там место было изначально.

Появился и стиль для бумаги. Раньше печать выносила на лист всю панель настроек;
теперь панель, кнопки и исходные виды на бумагу не попадают, а арт печатается
чернилами по белому, каким бы тёмным ни был экран.

Экраны с малой высотой теперь отдают понемногу отовсюду выше кнопки: арт берёт
34vh вместо 44, исходники ужимаются до миниатюр в 52 пикселя, ряды сдвигаются.
Арт остаётся самым крупным на экране — это тоже проверяется, — и оба размера
теперь укладывают кнопку с запасом: 514 из 568 и 370 из 390.
...
### Как сохраняются работы

Два места, потому что они отвечают на разные вопросы. **«Сохранить здесь»**
кладёт работу в собственную базу браузера: настройки, арт как он есть и картинку,
из которой он сделан, — без последней сохранённую работу можно было бы
разглядывать, но не продолжать, а это не то, что значит «сохранить».
**«В файл»** пишет то же самое одним JSON, и картинка лежит внутри него как
data URL — именно это делает его файлом, а не форматом архива. Открывается он
перетаскиванием на страницу — тем же жестом, что и картинка внутри него, — или
кнопкой, если удобнее выбрать в диалоге. Base64 стоит
трети лишних байт; не тащить ради этого библиотеку архивов того стоит.

Галерея — IndexedDB, а не `localStorage`: фотография весит мегабайты, а в
`localStorage` их всего пять на всё и только строками. И тем более не куки: куки
уходят на сервер с каждым запросом, то есть для статического сайта — отдают
хозяину хостинга то, над чем человек работает, а четыре килобайта это не
картинка.

Каждая сохранённая строка умеет уйти и сама по себе — кнопкой **«Файлом»**,
рядом с «Открыть» и «Удалить». Работа уезжает вместе с картинкой внутри; стиль
уезжает тем, чем он является, и возвращается тем же — поэтому в файле записано,
что именно в нём лежит. Без этого стиль вернулся бы работой без картинки и без
размера, а такого приложение показать не может.

У каждой сохранённой работы есть маленький рисунок её самой, сделанный той же
функцией, что рисует страницу и PNG, — поэтому миниатюра не может показать того,
чего в арте нет. Название и размер сетки различают две работы только тогда, когда
они различаются; картинка арта различает всегда, а в сотню пикселей ширины форма
фотографии ещё читается. Стоит это пару килобайт рядом с исходником в мегабайты.
Размер возле счётчика — это то, что занимают сами работы, а не весь сайт:
офлайн-кеш весит десятки мегабайт и утопил бы их в себе.

**«Сохранить стиль»** кладёт ту же запись, но без картинки и без арта: как
сделать, а не что сделано. Она попадает в тот же список, помечается как стиль и
миниатюры не несёт — показывать нечего; открыть её значит наложить эти настройки
на ту картинку, которая открыта сейчас. Различие того стоит: сохранённая работа —
то, что вы сделали, сохранённый стиль — то, как вы это делаете.

Раздел прямо говорит, что браузер — небезопасное место, чтобы что-то там
оставлять. Приватное окно забудет, «очистить данные сайта» унесёт, а iOS
выбрасывает всё, что сайт хранил, через семь дней без визита, если сайт не
установлен. Поэтому файл существует рядом с галереей, а не вместо неё.

Аккаунта здесь нет, и быть не может: статическая страница не в состоянии войти в
GitHub. Web-flow требует client secret, то есть сервера; device flow не пускает
браузер к обмену токена. Оставшийся вариант — попросить вставить personal access
token в поле на странице — хуже, чем кажется. GitHub хостит приложение и отдаёт
его офлайн как PWA; куда работа поедет дальше — дело файла.
...
поспевает ли пересчёт и сколько стоил последний, что делает чистка — и **что
именно унесёт копирование текстом**. Последнее там потому, что ошибиться легко, а
картинка на экране ошибку прячет: цвет вставку не переживает никогда. Обычному
арту это не вредит — его несут сами точки; мозаике смертельно, потому что её
точки говорят, к какой из двух красок относится место, а не насколько там светло.
Простым текстом мозаика — серая каша. Блоки доедут, но лишь настолько, насколько
у читающего есть для них шрифт. Копирование и сохранение текстом говорят то же
самое в тот момент, когда это важно, а не только в разделе. То, чего приложение
не решало само, так и говорит — пропущенная строка читалась бы как неисправность.
...
Текстом брайлевая ячейка — один глиф и несёт один цвет, поэтому непогашенные
точки это просто страница. В них и живёт основная ошибка цвета: по замеру на
шести картинках, как среднее расстояние CIE между точкой и цветом, который у неё
должен был быть, одна краска на общем фоне промахивается на **24–36**. Картинка
всё равно читается — потому что верен тон, а не цвета.

Там, где кроме чернил есть фон — страница, PNG, файл HTML, терминал, — ячейка
может нести две краски, и тот же замер падает до **4.3–7.9**. С точками при этом
не происходит ничего: их по-прежнему решает одна яркость, а флажок **и цвет за
ними** говорит лишь, что делать с теми, которые остались погашены. Раскраске
пробегами пришлось научиться тому, что теперь у пробега должны совпасть оба
цвета, иначе чернила одной ячейки лягут поверх фона другой.

Если пойти дальше — флажок **пусть цвет выбирает точки** — восемь точек ячейки
делятся между двумя красками, которые к ним ближе всего: k-means при k=2 в
L\*a\*b\*, начиная с двух самых далёких друг от друга, что для восьми точек не
оптимизация, а горстка сравнений. Выходит **2.0–3.8** — ближе всех трёх, — и это
отдельный режим, потому что это другая картинка по существу.

Точки перестают нести тон. Каждая ячейка делится между своими двумя красками,
поэтому примерно половина точек поднята где угодно: по замеру на пейзаже обычный
арт закрашивает 0.30 точек с разбросом 0.28 от ячейки к ячейке, а этот — 0.51 с
разбросом 0.18. На ровном градиенте разброс падает с 0.50 до 0.00. Вставленный
туда, где цвета нет, он серая каша; с цветом — самая точная картинка, какую эта
сетка может нести. Остальная панель к нему не относится: метод, порог и контуры
отвечают на вопросы, которых он не задаёт. Поднятой становится более светлая из
двух групп — чтобы инверсия значила то же, что и везде.

Чего приём не умеет — помочь чату: у обычного текста фона нет, и вставка не
меняется. Он для картинки, страницы и терминала.
...
Медианный срез — это начало палитры, а не её конец. Он выбирает коробки и ставит
запись в среднее каждой; следующего вопроса он не задаёт — раз записи встали
именно так, какие цвета к какой из них на самом деле относятся и стоит ли запись
всё ещё посреди них? С 0.52 этот вопрос задаётся: дюжину раз или пока ничего не
двигается. Замер на шести картинках и шести размерах сетки: десятая часть ошибки
цвета уходит — 12.91 против 11.64 средней ΔE, — и это лучше в 36 случаях из 36 и
не хуже ни в одном.

Середина кластера — это среднее **в L\*a\*b\***, а не в линейном свете. Выглядит
как противоречие правилу, которое здесь соблюдается везде, но им не является:
свет усредняют, когда ответ обязан светить как то, что он собой заменяет, — так
устроен собственный цвет ячейки. Здесь же ответ обязан встать как можно ближе к
набору цветов в том пространстве, в котором меряется расстояние, а это и есть
среднее в этом пространстве.

Подгонка идёт по восьми тысячам ячеек, а не по всем, и берутся они шагом, чтобы
одна и та же картинка всегда давала одну и ту же палитру: по замеру подгонка по
выборке не хуже полной (11.64 против 11.77) и превращает 311 мс в 15 мс на самой
большой сетке, которую приложение нарисует. Подгоняется она по обоим цветам
ячейки, чернилам и фону, потому что к ней привязываются оба: палитра, видевшая
одни чернила, могла бы оставить фон ячейки на цвете, за которым в картинке
ничего нет.
...
**«Определить самому»** делает после выбора пресета ещё одно: спрашивает,
не отдаёт ли картинка арту больше диапазона, чем тот способен показать. Не по
тому, насколько картинка тёмная — рисунок на белом фоне почти весь одного
значения, и трогать его не надо, — а по тому, сколько из принесённого
выбрасывается. Считаются ячейки, которые арт рисует как ничто (все точки внизу
или все вверху) **там, где картинке было что показать**. У чистого белого фона
фактуры нет, он не считается — именно это и не даёт приёму его испортить.

Величина подъёма выбирается перебором с замером, а не правилом про яркость:
0, 40, 70, 100 — и берётся наименьшая из тех, что не хуже лучшей. Молчать
заставляют два порога: приём срабатывает, только если теряется хотя бы одна
фактурная ячейка из двадцати, и только если подъём отыгрывает пятую часть
потери. По замеру на шести настоящих картинках это поднимает тёмный лес и
пейзаж — они теряли 51% и 25% фактуры — и не трогает остальные четыре, где
подъём отыгрывал от 7% до 17%, чего мало, чтобы двигать контрол на глазах у
человека.

Что бы он ни решил, это видно на ползунке, сказано в строке состояния и
записано в разделе «что решено». Рука на ползунке забирает авторство обратно:
раздел начинает писать, что тени подняты вручную.
