...
| **Подбор вариантов** | считает набор рецептов, оценивает каждый по картинке, предлагает четыре лучших |
...
У этого есть граница, и её стоит назвать вслух: разделяются слабое-связное и
слабое-одинокое, поэтому текстура, отвечающая не слабее контура, приёму не
поддаётся. На фотографии леса крапины Собеля по-настоящему сильные — каждая
хвоинка и есть настоящий перепад, — и чистка их почти не трогает.
...
запланирована и не построена: она лечит разорванные контуры, а измерение
показало обратную беду — 93–97% чернил XDoG и так лежали в длинных штрихах.
Рвать было нечего, чернил было слишком много.
...
каждого. Кнопка **«Подобрать»** считает десять рецептов, оценивает каждый по
...
Одна находка со стройки стоит того, чтобы её передать. На здешних ширинах — 40 и
60 символов — упорядоченный дизеринг и синий шум нередко обыгрывают
Флойда–Стейнберга на фотографиях, чего при печатных разрешениях не бывает.
Диффузия ошибки меняет пространственную точность на тональную, и на такой грубой
сетке размен перестаёт окупаться: ошибка уезжает достаточно далеко, чтобы
сдвинуть детали. Ровно такие вещи эта кнопка и должна показывать.
